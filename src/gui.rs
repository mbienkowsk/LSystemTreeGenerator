...
impl GuiController {
    pub fn new(
        ...
        Self {
            ...
            l_system_config: LSystemConfig::default(),
    ...
    pub fn draw(&mut self, window: &Window, display: &Display<WindowSurface>, frame: &mut Frame) {
        ...
        let lsystem_config = &mut self.l_system_config;
        ...
        self.egui_glium.run(window, |ctx| {
            egui::Window::new("Control panel").show(ctx, |ui| {
                ...
                ui.separator();
                GuiController::ui_l_system_config(lsystem_config, preset_selection, ui);
                ui.separator();
    ...
    pub fn get_l_system_config(&self) -> &LSystemConfig {
        &self.l_system_config
    }
    ...
    fn ui_l_system_config(
        l_system_config: &mut LSystemConfig,
        ...
        if old_preset != *preset_selection && *preset_selection != PresetSelection::Custom {
            *l_system_config = preset_selection.to_config();
        }

        ui.separator();
        ui.label("LSystem Config:");
        ui.add(
            egui::Slider::new(&mut l_system_config.n_iterations, 0..=6)
                .text("Number of Iterations"),
        );
        ui.add(egui::Slider::new(&mut l_system_config.angle, 0.0..=45.0).text("Angle"));

        ui.horizontal(|ui| {
            ui.label("Axiom:");
            if ui
                .text_edit_singleline(&mut l_system_config.axiom)
                .changed()
            {
        ...
        for (i, (symbol, replacement)) in l_system_config.production_rules.iter_mut().enumerate() {
            ui.horizontal(|ui| {
        ...
        if let Some(idx) = to_remove {
            l_system_config.production_rules.remove(idx);
        }
...
impl Default for LSystemConfig {
    fn default() -> Self {
        PresetSelection::default().to_config()
    }
}
...
impl PresetSelection {
    fn to_config(self) -> LSystemConfig {
        match self {
